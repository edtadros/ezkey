...
add(
    ...
    faqs=[
        ...
        ("Is Always Allow safe?", "Always Allow asks macOS to stop prompting ezkey for that item, but macOS may still ask again. ezkey does not suppress or work around that prompt. That is on purpose: you get the same protection as Keychain Access. Only run ezkey you built yourself."),
    ],
    body=[
...
add(
    ...
    body=[
        ...
        ("p", "ss64 and `man security` document the rest. Creator codes and `-T` trusted apps are how Access Control lists get weird. Default trust includes the creating app. Always Allow asks macOS to add ezkey to that list, but macOS may still ask again."),
...
add(
    ...
    body=[
        ...
        ("p", "It will not rotate keys at OpenAI. It will not sync to a phone. It will not hide from a process running as you. It will not replace 1Password for your bank. You build it from source. Those are not missing checkboxes. They are the product boundary. See [anti-cloud local keys](/for/local-api-keys/)."),
        ("h2", "Start from source", "start"),
        ("pre", "git clone --depth 1 --branch <tag> https://github.com/edtadros/ezkey.git\ncd ezkey\n./scripts/install.sh"),
        ("p", "Replace `<tag>` with the newest `v*` release tag. To review the code before building, follow the [build-ezkey skill](https://ezkey.app/.well-known/agent-skills/build-ezkey/SKILL.md). Requires macOS 14+ and Xcode command-line tools. Then follow [how to save API keys securely](/guides/how-to-save-api-keys-securely-on-mac/)."),
...
add(
    ...
    body=[
        ...
        ("p", "Certificates, code signing identities, looking at Access Control, inspecting iCloud items. ezkey will not grow into that. If you need those tools, open Access. That is not a failure of ezkey. It is the division of labor."),
