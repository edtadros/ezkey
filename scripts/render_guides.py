...
add(
    ...
    faqs=[
        ...
        ("Is Always Allow safe?", "Always Allow is a standing grant to the code signature of the build you compiled. Each rebuild changes that signature, so macOS may ask again after an update. Do not give it to a downloaded .app."),
    ],
    body=[
...
add(
    ...
    faqs=[
        ...
        ("CI artifacts?", "Tests only. CI does not attach an .app. CI artifacts are not a release channel."),
    ],
    body=[
        ...
        ("p", "It will not rotate keys at OpenAI. It will not sync to a phone. It will not hide from a process running as you. It will not replace 1Password for your bank. It will not ship a prebuilt binary. You build it from source. Those are not missing checkboxes. They are the product boundary. See [anti-cloud local keys](/for/local-api-keys/)."),
